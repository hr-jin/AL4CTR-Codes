...
def din_attention(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False, att_score=None, name_scope=''):
  ...
  d_layer_1_all = tf.layers.dense(din_all, 80, activation=tf.nn.sigmoid, name=name_scope+'f1_att' + stag)
  d_layer_2_all = tf.layers.dense(d_layer_1_all, 40, activation=tf.nn.sigmoid, name=name_scope+'f2_att' + stag)
...
def se_block(input, emb_dim, name_scope):
  se_ratio = 4
...
def self_agg(seq_embs, mask, scope="self_agg"):
  padding_num = -2**30 + 1
  with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
    dnn1 = tf.layers.dense(seq_embs, 36, activation=None, name="f1")
    dnn1 = prelu(dnn1, 'prelu1')

    dnn2 = tf.layers.dense(dnn1, 1, activation=None, name="f2")
    
    dnn2 = tf.transpose(dnn2, [0,2,1])
    key_mask = tf.expand_dims(mask, 1)
    paddings = tf.ones_like(dnn2) * padding_num

    weights = tf.where(tf.cast(key_mask, tf.bool), dnn2, paddings)
    self_scores = tf.nn.softmax(weights, axis=-1)
    self_interest = tf.matmul(self_scores, seq_embs)

    self_interest = tf.squeeze(self_interest, [1])
